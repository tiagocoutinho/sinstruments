...
To start the server you can do something like::
    ...
    $ python -m isim -c my_simulator.yml
"""
...
__all__ = ["Server", "SimulatorServerMixin", "SerialServer", "TCPServer"]
...
class SerialServer(BaseServer, SimulatorServerMixin):
    ...
    def set_listener(self, listener):
        ...
        self._log.info('Created symbolic link "%s" to simulator pseudo ' \
...
class Server(object):
    ...
    def create_device(self, device_config):
        klass_name = device_config.get("class")
        name = device_config.get("name", klass_name)
        self._log.info("creating device %s (%r)", name, klass_name)
        device, transports = create_device_from_config(device_config)
...
def create_device_from_config(config):
    device_info = dict(config)
...
def create_server_from_config(config):
    ...
    name = config.get("name", "simulator")
...
def run(args=None):
    ...
    parser.add_argument(
        "-c", "--config-file", default='./isim.yml',
